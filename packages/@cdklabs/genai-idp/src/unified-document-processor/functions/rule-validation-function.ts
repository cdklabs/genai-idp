...
/**
 * Properties for the Rule Validation function.
 *
 * This function validates document sections against configured rules using
 * Bedrock model invocation. It supports optional Bedrock Guardrails for
 * content filtering.
 *
 * @since 0.5.2
 */
export interface RuleValidationFunctionProps extends IdpPythonFunctionOptions {
  ...
  /**
   * Optional inference provider for rule validation.
...
/**
 * Lambda function that validates document sections against configured rules.
 *
 * This function uses Bedrock model invocation to validate extracted data
 * against user-defined rules for each document section. It runs within
 * a Map state in the Step Functions workflow, processing sections in parallel.
 ...
 * @since 0.5.2
 */
export class RuleValidationFunction extends lambda_python.PythonFunction {
  constructor(
    scope: Construct,
    id: string,
    ...
    super(scope, id, {
      ...
      entry: path.join(
        __dirname,
        "../../../assets/lambdas/unified/rule-validation-function",
      ),
      bundling: {
        command: [
          "bash",
