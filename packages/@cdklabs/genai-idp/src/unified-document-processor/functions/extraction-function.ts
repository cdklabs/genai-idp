...
/**
 * Properties for the Extraction function.
 *
 * This function extracts structured data from documents using Bedrock model
 * invocation. It supports optional Bedrock Guardrails for content filtering.
 *
 * @since 0.5.2
 */
export interface ExtractionFunctionProps extends IdpPythonFunctionOptions {
  ...
  /**
   * The S3 bucket for extraction configuration files.
   */
  ...
  /**
   * Optional inference provider for extraction.
   ...
   * When provided, the extraction function invokes this Lambda for custom prompt generation.
   *
   * @default - No custom prompt generator
   */
  readonly customPromptGenerator?: lambda.IFunction;
}

/**
 * Lambda function that extracts structured data from documents.
 *
 * This function uses Bedrock model invocation to extract structured information
 * from classified document sections. It supports optional Bedrock Guardrails
 * for content filtering during extraction.
 *
 * Environment variables:
 * - METRIC_NAMESPACE: CloudWatch metric namespace (stack name)
 * - CONFIGURATION_BUCKET: S3 bucket for extraction configuration
 ...
 *
 * Note: This function needs Bedrock model invoke, ConfigurationBucket read,
 * and optional Guardrail apply permissions, granted in the parent construct.
 *
 * @since 0.5.2
 */
export class ExtractionFunction extends lambda_python.PythonFunction {
  constructor(scope: Construct, id: string, props: ExtractionFunctionProps) {
    ...
    super(scope, id, {
      ...
      layers: [IdpPythonLayerVersion.getOrCreateForArchitecture(scope, lambda.Architecture.ARM_64, "extraction", "docs_service")],
