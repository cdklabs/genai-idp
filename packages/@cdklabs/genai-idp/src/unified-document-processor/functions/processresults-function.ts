 ...
 *
 * This function processes pipeline results and writes final output.
 * It has an explicit OUTPUT_BUCKET environment variable for writing results.
 *
 * @since 0.5.2
 */
...
/**
 * Lambda function that processes pipeline results and writes final output.
 *
 * This function handles the final step of the pipeline processing branch,
 * collecting results from extraction and assessment steps and writing them
 * to the output bucket. It needs AppSync GraphQL permissions for both
 * Mutation and Query types when AppSync tracking is enabled.
 *
 ...
 * - OUTPUT_BUCKET: S3 bucket for final processed output
 * - CONFIGURATION_TABLE_NAME: Configuration table name
 *
 * Note: This function needs AppSync GraphQL permissions when AppSync tracking
 * is enabled, granted in the parent construct.
 *
 * @since 0.5.2
 */
export class ProcessResultsFunction extends lambda_python.PythonFunction {
  constructor(
    scope: Construct,
    id: string,
